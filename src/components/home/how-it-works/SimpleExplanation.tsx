          ...
          {STEPS.map((step, index) => (
            <div key={step.title} className="contents">
              <div className="group flex flex-col items-center text-center flex-1 space-y-4">
                <div className={`w-20 h-20 rounded-4xl ${step.colorClass} border flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform duration-500`}>
                  <span className="text-3xl">{step.emoji}</span>
                </div>
                <div className="space-y-1">
                  <p className="text-white font-bold tracking-tight">{step.title}</p>
